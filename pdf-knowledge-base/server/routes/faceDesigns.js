...
router.post('/preview', (req, res) => {
  const { name, grid, openGrid, color } = req.body || {};
  // Unsaved designs can be previewed too: the frames are sent straight through.
  const isNeutral = String(name || '').toLowerCase() === 'neutral';
  const payload = grid === undefined
    ? getDevicePayload(getFaceByName(name)?.name || name)
    : isNeutral
    ? { setEmotion: 'neutral', neutralFace: { grid, openGrid: openGrid || grid, color: color || '4CFF7A' } }
    : { setEmotion: String(name || 'preview').slice(0, 22), face: { grid, openGrid: openGrid || grid, color: color || '4CFF7A' } };
  const shown = payload.face || payload.neutralFace;
  if (shown) {

...
import { ArrowLeft, Layers, Sun, Moon } from 'lucide-react';

const getShortSummary = (text) => {
  if (!text) return '';
  let cleanText = text.replace(/^\[SYNCED INTEL FROM [^\]]+\]\s*/i, '');
...
export const SunburstCanvas = ({
  ...
  const handleSliceClick = (e, slice) => {
    ...
    clickTimerRef.current = setTimeout(() => {
      ...
      // Leaf nodes on single-click do nothing (double-click opens panel)
    }, DBLCLICK_DELAY_MS);
  };
  ...
  return (
    ...
    >
        ...
        >
          ...
          <g>
            {flatSlices.map(slice => {
              ...
              // entityColor: this node's own type — used for background and hover glow
              const entityColor = (ENTITY_TYPES[slice.type?.toUpperCase()] || ENTITY_TYPES.CONCEPT).color;
              const fillColor   = entityColor;
          ...
          {/* Hover edge glow — on top of fills for crisp outline */}
          {hoveredNode && (() => {
            const { r0, r1 } = getRadius(hoveredNode.depth);
            const pathD = getArcPath(r0, r1, hoveredNode.startAngle, hoveredNode.endAngle);
            const color = (ENTITY_TYPES[hoveredNode.type?.toUpperCase()] || ENTITY_TYPES.CONCEPT).color;
            return (
          ...
          <g pointerEvents="none">
            {secondaryPaths.map(p => (
              <path key={p.id} d={p.d} style={{ fill: 'none', stroke: p.color, strokeWidth: 1.5, strokeDasharray: '4,4', strokeOpacity: 0.45 }} />
            ))}
          </g>

          {/* Labels — SVG tspan wrapping so full titles show on 2 lines */}
          <g pointerEvents="none">
            {flatSlices.map(slice => {
              // Wrap a title string into lines of ≤ maxChars, max 3 lines
              const wrapLabel = (title, maxChars = 11) => {
              ...
              const LINE_H = 9; // px between tspan baselines
              ...
              if (slice.depth === 0) {
                ...
                return (
                  <text
                    ...
                    style={{ ...fillStyle, fontSize: '9px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.06em' }}
                  >
                    {lines.map((line, i) => (
              ...
              // For outer rings allow tighter threshold so more labels appear
              const minSweep = slice.depth === 1 ? 0.10 : 0.08;
              if (sweep < minSweep) return null;
              ...
              // How many chars fit depends on ring width and depth
              const charsPerLine = slice.depth === 1 ? 12 : 10;
              ...
              return (
                <text
                  ...
                  style={{ ...fillStyle, fontSize: '7px', fontWeight: 700, letterSpacing: '0.02em' }}
                >
                  {lines.map((line, i) => (
                    <tspan
                      key={i}
      ...
      >
        {/* Theme toggle row — top-right, minimal */}
        {onThemeToggle && (
          <div className="flex justify-end -mt-6 mb-1">
            <button
              onClick={onThemeToggle}
              title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
              className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border text-[9px] font-bold uppercase tracking-wider transition-all active:scale-95 ${
                isDark
                  ? 'border-white/8 text-slate-500 hover:text-brand-cyan hover:border-brand-cyan/30 bg-white/3'
                  : 'border-[#2E2B27]/12 text-[#9A948E] hover:text-[#2E2B27] hover:border-[#2E2B27]/25 bg-black/3'
              }`}
            >
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: 'Nodes', value: treeData?.value || 0 },

...
/**
 * OnboardingSetup Component.
 * 
 * DESIGN RATIONALE:
 * - Guides the user through a cohesive multi-step setup flow: Auth → Root Folder Selector → Config Settings.
 * - Adheres strictly to the regionalisation policy using British English and GBP (£) for limits.
 * - Embedded layout features glassmorphism panels, elegant transitions, and Oatmeal custom property support.
 * 
 * @param {Object} props Component properties.
 * @param {Object} props.authStatus Current authorization configuration.
 * @param {Function} props.onComplete Callback executed when setup is successfully saved.
 * @param {string} props.API API server endpoint base URL.
 */
export default function OnboardingSetup({ authStatus, onComplete, API }) {
  ...
  const [spendCap, setSpendCap] = useState(250); // Expressed in GBP (£) as per policy
  const [model, setModel] = useState('flash');
  const [loadingFolders, setLoadingFolders] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState(null);

  // Check URL query parameters for connection outcomes
  useEffect(() => {
  ...
  // Fetch folders under active step 2 context
  useEffect(() => {
  ...
  /**
   * Fetches folder collection children.
   * 
   ...
   * Enters child folder.
   * 
   * @param {string} folderId Destination folder identifier.
  ...
  /**
   * Returns backward to previous breadcrumb directory.
   * 
   * @param {number} index Index inside the path trail array.
   */
  ...
  /**
   * Saves settings to registry and terminates the onboarding wizard.
   */
  ...
  return (
    <div className="onboarding-overlay">
      <div className="onboarding-card">
        ...
        <h2>Corporate Intelligence Portal</h2>
        <p>Connect your Google Drive to configure the workspace and access your corporate knowledge base with spatial RAG pipelines.</p>
        ...
        <div className={`onboarding-step ${step > 1 ? 'completed' : ''}`}>
          ...
          <div className="onboarding-step-content">
            ...
            <p>{authStatus.email
              ...
              : 'Sign in with Google to access the Information management system workspace'}</p>
          </div>
        </div>
          ...
          <>
            ...
            {step === 2 && (
              <>
                ...
                {/* Folder picker results container */}
          ...
          <>
            <div className="onboarding-step">
              ...
              <div className="onboarding-step-content">
                ...
                <p>Set your budget limit and preferred AI model</p>
              </div>
            </div>

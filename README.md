...
| `/ims/facedesigner` | **Face Designer.** Every face Ims can pull, as two 12x8 dot grids (mouth closed / open). Click dots to light them (click again to switch off), open the colour picker or type a hex colour, name the face, and write *when it should be used* - that text is what Gemini is told. Every face is editable. **Standby** is Ims's idle face (never chosen by Ims; listening, thinking and connecting keep their state colours). Every face has *Blink* and *Glance* options that reuse the standby eye movement: a blink flattens the eyes, and a dim pupil dot slides sideways. The faded breathing background dots always take a dim version of the face's colour. Preview on IMS |

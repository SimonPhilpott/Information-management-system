...
export default function App() {
  ...
  const [nodes, setNodes] = useState(() => {
    // VERSIONED STORAGE KEY: hive_mesh_v16_corporate_hierarchy
    // v16: bumped to force clean re-seed of the deduplicated database files.
    // Migrates any user-created nodes from v15 so custom work is not lost.
    const STORAGE_KEY = 'hive_mesh_v16_corporate_hierarchy';
    const saved = localStorage.getItem(STORAGE_KEY);

    // Migration: pull in user-created nodes from previous v15 key so they are not lost.
    let migratedUserNodes = [];
    if (!saved) {
      try {
        const old = localStorage.getItem('hive_mesh_v15_corporate_hierarchy');
        if (old) {
    ...
    try {
      ...
      return { 
       ...
       return { 
          ...
          showLabels: true, labelStyle: 'standard', showHeatmap: false, betaLayout: true
       };
     }
   });
